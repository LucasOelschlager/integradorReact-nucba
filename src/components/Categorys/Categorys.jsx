...
export const Categorys = () => {
    const { categorySet, setCategorySet } = useContext(CategoryContext)
    return (
        <>
            ...
            <div className={`${styledCategorys.container}`}>
                <ul className={`${styledCategorys.list}`}>
                    {categorias.map((e) => (
                        <li key={e.id} className={categorySet == e.name.toLowerCase() ? `${styledCategorys.categoryItemAct}` : styledCategorys.categoryItem} onClick={() => setCategorySet(e.name.toLowerCase())}>
